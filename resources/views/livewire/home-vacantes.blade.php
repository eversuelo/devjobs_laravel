        ...
        <div class="max-w-7xl mx-auto">
            ...
            <div class="bg-white shadow-sm rounded-lg p-6 divide-y divide-gray-200">
                @forelse ($vacantes as $vacante)
                    <div class="md:flex justify-around md:items-center py-5">
                        <div class="md:flex-1">
                            <a class="text-3xl font-extrabold text-gray-600" href="{{ route('vacante.show', $vacante->id) }}">{{ $vacante->titulo }}</a>
                            ...
                            <p class="font-bold text-xs text-gray-600">
                                Último día para postularme: <span class="font-normal">{{ $vacante->ultimo_dia->format('d/m/Y') }}</span>
                            </p>
                        </div>

                        <div class="mt-5 md:mt-0">
                            <a class="bg-indigo-500 p-3 text-sm uppercase font-bold text-white rounded-lg block text-center"
                                href="{{ route('vacante.show', $vacante->id) }}">
                                Ver Vacante
                            </a>
                        </div>
                    </div>
